...
// } from 'react-icons/fa';
// import {
//     getProducts, getCategories, getSales, deleteProduct, createProduct,
...
//                 console.log(`Page ${page}: Got ${productsData.length} products, Total: ${allProducts.length}`);
...
//             // Fallback: try to get all products without pagination
...
//             try {
//                 const allProducts = await fetchAllProductsWithPagination(merchantId);
...
//             // Fetch sales
//             try {
//                 const salesRes = await getSales(merchantId);
...
//             // Fetch users
//             try {
//                 const usersRes = await getUsers(merchantId);
...
//             // Fetch carts for users
//             if (users.length > 0) {
//                 try {
//                     const userList = users.length > 0 ? users : [];
//                     if (userList.length > 0) {
//                         const cartPromises = userList.map(u =>
...
//                         console.log('Carts loaded:', allCarts.length);
//                     } else {
//                         setCarts([]);
//                     }
//                 } catch (err) {
...
//         return user ? `${user.first_name || ''} ${user.last_name || ''}`.trim() || user.email || 'N/A' : userId.substring(0, 8) + '...';
//     };
...
//                                             </tr>
//                                         ))}
//                                         {products.length === 0 && <tr><td colSpan="6" className="py-6 text-center text-gray-500">No products yet</td></tr>}
//                                     </tbody>
//                                 </table>
//                             </div>
//                         </div>
...
//                                         </tr>
//                                     ))}
//                                     {products.length === 0 && <tr><td colSpan="5" className="py-6 text-center text-gray-500">No products yet</td></tr>}
//                                 </tbody>
//                             </table>
//                         </div>
//                     </div>
...
//                                             <td className="py-3 text-sm font-medium">{getUserName(sale.user_id)}</td>
...
//                                             <td className="py-3"><button className="text-blue-600 hover:text-blue-800"><FaEye /></button></td>
//                                         </tr>
//                                     ))}
//                                     {sales.length === 0 && <tr><td colSpan="5" className="py-6 text-center text-blue-800 italic">No orders yet</td></tr>}
//                                 </tbody>
//                             </table>
//                         </div>
//                     </div>
...
//                     <div className="bg-white rounded-lg shadow-sm p-6">
//                         <div className="flex justify-between items-center mb-4">
//                             <h3 className="text-lg font-semibold text-blue-800 italic">Users ({Array.isArray(users) ? users.length : 0})</h3>
...
//                                                         <div className="flex gap-2">
//                                                             <button
//                                                                 onClick={() => viewUserDetails && viewUserDetails(user.id || user._id)}
...
//                                                             </button>
//                                                             <button
//                                                                 onClick={() => editUser && editUser(user)}
...
//                                                             </button>
//                                                             <button
//                                                                 onClick={() => deleteUser && deleteUser(user.id || user._id)}
...
//                                             <td className="py-3">
//                                                 <div className="flex gap-2">
//                                                     <button onClick={() => viewCart(cart)} className="text-blue-600 hover:text-blue-800 text-sm"><FaEye /></button>
//                                                 </div>
//                                             </td>
//                                         </tr>
//                                     )) : (
...
const AdminDashboard = () => {
    ...
    const fetchAllData = async (merchant) => {
        ...
        try {
            ...
            // Fetch carts in background (don't block loading)
            if (allUsers.length > 0) {
                const userList = allUsers.slice(0, 50);
                const cartPromises = userList.map(u =>
                    getCart(u.id || u._id).catch(() => ({ data: null }))
                );
                const cartResponses = await Promise.all(cartPromises);
    ...
    const getUserByUserId = (userId) => {
        return users.find(u => (u.id || u._id) === userId);
    };
    ...
    return (
        <div className="min-h-screen bg-gray-100">
            ...
            <div className="lg:ml-64 p-6">
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full">
                                    ...
                                    <tbody>
                                        ...
                                        {products.length === 0 && (
                                            <tr>
                                                <td colSpan="6" className="py-6 text-center text-gray-500">No products yet</td>
                                            </tr>
                                        )}
                                    </tbody>
                                </table>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                ...
                                <tbody>
                                    ...
                                    {products.length === 0 && (
                                        <tr>
                                            <td colSpan="5" className="py-6 text-center text-gray-500">No products yet</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                ...
                {/* Orders Tab - Shows customer names correctly */}
                {activeTab === 'orders' && (
                    <div className="bg-white rounded-lg shadow-sm p-6">
                        <h3 className="text-lg font-semibold text-blue-800 italic mb-4">All Orders ({sales.length})</h3>
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                ...
                                <tbody>
                                    {sales.map((sale, idx) => {
                                        const user = getUserByUserId(sale.user_id);
                                        return (
                                            <tr key={sale.id || sale._id} className="border-b hover:bg-gray-50">
                                                <td className="py-3 text-sm">{idx + 1}</td>
                                                <td className="py-3 text-sm font-medium">
                                                    {user ? `${user.first_name || ''} ${user.last_name || ''}`.trim() : (sale.customer_name || 'N/A')}
                                                </td>
                                                <td className="py-3 text-sm">{user?.email || 'N/A'}</td>
                                                <td className="py-3 text-sm">${sale.amount}</td>
                                                <td className="py-3">
                                    ...
                                    {sales.length === 0 && (
                                        <tr>
                                            <td colSpan="7" className="py-6 text-center text-blue-800 italic">No orders yet</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                                        ...
                                        users
                                            ...
                                            .map((user, i) => (
                                                <tr key={user.id || user._id || i} className="border-b hover:bg-gray-50">
                                                    ...
                                                    <td className="py-3">
                                                        <div className="flex gap-2">
                                                            <button onClick={() => viewUserDetails(user.id || user._id)} className="text-blue-600 hover:text-blue-800"><FaEye /></button>
                                                            <button onClick={() => editUser(user)} className="text-green-600 hover:text-green-800"><FaEdit /></button>
                                                            <button onClick={() => deleteUser(user.id || user._id)} className="text-red-600 hover:text-red-800"><FaTrash /></button>
                                                        </div>
                                                    </td>
                                                </tr>
                                            ))
                            ...
                            <table className="w-full">
                                ...
                                <tbody>
                                    {carts.length > 0 ? carts.map((cart) => (
                                        ...
                                        <tr>
                                            <td colSpan="5" className="py-12 text-center text-blue-800 italic">No active carts found</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
            ...
            {/* Cart Detail Modal - No background */}
            {showCartModal && selectedCart && (
            ...
            {/* User Details Modal - No background */}
            {showUserModal && selectedUser && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'transparent' }}>
                    <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[80vh] overflow-y-auto">
                        <div className="p-6">
