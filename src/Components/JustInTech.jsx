    ...
    useEffect(() => {
        const fetchProducts = async () => {
            ...
            try {
                ...
                // Get categories to find the 'tech' category
                const categoriesRes = await getCategories(merchantId);
                const categories = categoriesRes.data?.data || categoriesRes.data || [];
                console.log('Categories:', categories);

                // Try different possible tech category names
                ...
                console.log('Tech category:', techCategory);

                if (!techCategory) {
                    console.log('No tech category found. Create a category named "tech" or "electronics" in the admin.');
                    setLoading(false);
                    return;
                }

                // Get products for tech category
                const categoryId = techCategory.id || techCategory._id;
                ...
                console.log('Tech products:', allProducts);
        ...
        return (
            <div className="max-w-[1400px] mx-auto px-5 py-8">
                ...
                <div className="flex justify-center py-8">
                    <div className="animate-pulse text-gray-400">Loading tech items...</div>
                </div>
            </div>
        );
    }
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    {visibleProducts.map((product) => (
                        ...
                        >
                            <JustInFoodCard
                                image={product.images?.[0]}
                                tag={product.is_best_seller ? "Best seller" : (product.is_reduced_price ? "Reduced price" : null)}
                                price={product.price}
                                originalPrice={product.original_price || null}
